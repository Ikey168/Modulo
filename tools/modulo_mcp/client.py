...
class ModuloClient:
    ...
    def update_record(self, plugin_id: str, key: str, value: Any, expected_version: int,
                      schema_id: str | None = None, schema_version: int | None = None) -> dict[str, Any]:
        if type(expected_version) is not int or expected_version < 1:
            raise ModuloError("expected_version must be the positive version you inspected")
        record = self.get_record(plugin_id, key)
        if record["version"] != expected_version:
            raise ModuloError(f"Record changed: expected version {expected_version}, current version {record['version']}")
        schema_id = schema_id or record["schemaId"]
        schema_version = schema_version or record["schemaVersion"]
        if len(schema_id) > 256 or type(schema_version) is not int or schema_version < 1:
            raise ModuloError("A valid schema ID and positive schema version are required")
        return self._request(self._state_path(plugin_id, key), "PUT", {
            "expectedVersion": expected_version, "schemaId": schema_id,
            "schemaVersion": schema_version, "value": value,
        })

    def delete_record(self, plugin_id: str, key: str, expected_version: int) -> dict[str, Any]:
        if type(expected_version) is not int or expected_version < 1:
            raise ModuloError("expected_version must be the positive version you inspected")
        return self._request(f"{self._state_path(plugin_id, key)}?{urlencode({'expectedVersion': expected_version})}", "DELETE")

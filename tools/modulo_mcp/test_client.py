...
class StateHandler(BaseHTTPRequestHandler):
    ...
    def route(self):
        ...
        if self.command == "DELETE":
            expected = int(parse_qs(parsed.query)["expectedVersion"][0])
            self.writes.append((plugin, key, {"delete": expected}))
            if not current or expected != version:
                return self.respond(409, {"code": "STATE_VERSION_CONFLICT"})
            record = {**current, "version": version + 1, "value": None, "deleted": True}
            self.records[(plugin, key)] = record
            return self.respond(200, copy.deepcopy(record))
        body = json.loads(self.rfile.read(int(self.headers["Content-Length"])))
...
class ModuloClientTests(unittest.TestCase):
    ...
    def test_update_replaces_value_keeps_schema_and_detects_changes(self):
        self.client.create_record("para", "data", "modulo.workspace.para", {"areas": [1, 2]}, 3)
        updated = self.client.update_record("para", "data", {"areas": [1]}, 1)
        self.assertEqual((updated["version"], updated["schemaId"], updated["schemaVersion"]), (2, "modulo.workspace.para", 3))
        self.assertEqual(updated["value"], {"areas": [1]})
        with self.assertRaisesRegex(ModuloError, "Record changed"):
            self.client.update_record("para", "data", {"areas": []}, 1)
        with self.assertRaises(ModuloError):
            self.client.update_record("para", "data", {}, 0)
        self.assertEqual(len(StateHandler.writes), 2)

    def test_delete_requires_current_version(self):
        self.client.create_record("media", "item-1", "modulo.workspace.media-item", {"position": 0})
        with self.assertRaisesRegex(ModuloError, "STATE_VERSION_CONFLICT"):
            self.client.delete_record("media", "item-1", 5)
        deleted = self.client.delete_record("media", "item-1", 1)
        self.assertTrue(deleted["deleted"])
        self.assertEqual(StateHandler.writes[-1], ("media", "item-1", {"delete": 1}))

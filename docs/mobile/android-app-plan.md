...
The reproducible [catalog inventory](plugin-catalog-inventory.json) loads all 180 runnable plugins and records 177 views, 14 note panels, 8 fence renderers, 13 editor actions, and 14 blueprint nodes. The [storage inventory](plugin-storage-inventory.json) now finds 83 direct Storage references, 37 literal key declarations, and 41 shared write-helper calls in workspace/services source. These counts are a discovery baseline, not Android workflow passes: computed keys, native dependencies, and individual device behavior still need review. Regenerate them with `cd frontend && ../node_modules/.bin/vite-node scripts/inventoryPluginCatalog.ts ../docs/mobile/plugin-catalog-inventory.json` and `node frontend/scripts/inventoryPluginStorage.mjs` from the repository root.
...
CI now regenerates the catalog inventory, rejects new browser Storage references above the per-file baseline, and syncs the packaged Android shell. The baseline still contains 83 references and must be reduced to zero normal-plugin references before P19 can close.

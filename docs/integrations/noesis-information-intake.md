...
The plugin can record a standalone yes/no choice without creating a research project. The user enters the question, alternatives, stakes, required confidence, stop condition, uncertainty, optional missing inputs/deadline, selected option, and rationale. Noesis stores the finalized `noesis-decision-v2` record and its revisions; Modulo stores only the current decision ID/version in signed-in plugin state. The request key is saved before the create call so a lost response can be retried without a duplicate. Reload checks current Noesis access, and a denied read hides the record. A revision uses Noesis's expected revision and re-inspects after an uncertain response. The underlying Noesis decision store supports a separate sensitivity receipt, but the plugin does not yet render its comparative matrix or link the choice to a Modulo task/project. The form does not acquire evidence, and the user's choice is not a machine-validated recommendation.

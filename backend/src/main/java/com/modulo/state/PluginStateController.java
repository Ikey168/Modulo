...
@RestController
@RequestMapping(
    value = "/api/workspaces/{workspace}/plugin-state/{namespace}",
...
public class PluginStateController {
  ...
  public PluginStateController(PluginStateStore store) {
    this.store = store;
  }
  ...
  public ResponseEntity<List<PluginStateStore.Change>> changes(
      @PathVariable String workspace,
      @PathVariable String namespace,
      @RequestParam long changesAfter,
      @RequestParam(defaultValue = "100") int limit) {
    return ResponseEntity.ok()
        .cacheControl(CacheControl.noStore())
        .body(store().changes(workspace, namespace, changesAfter, limit));
  }

  @GetMapping("/{key}")
  public ResponseEntity<PluginStateStore.StateRecord> get(
      @PathVariable String workspace, @PathVariable String namespace, @PathVariable String key) {
  ...
  @Operation(
      requestBody =
          @io.swagger.v3.oas.annotations.parameters.RequestBody(
              required = true,
              content =
                  @Content(schema = @Schema(implementation = PluginStateStore.WriteRequest.class))))
  public ResponseEntity<PluginStateStore.StateRecord> put(
      @PathVariable String workspace,
      @PathVariable String namespace,
  ...
  @DeleteMapping("/{key}")
  public ResponseEntity<PluginStateStore.StateRecord> delete(
      @PathVariable String workspace,
      @PathVariable String namespace,
      @PathVariable String key,
  ...
  private ResponseEntity<PluginStateStore.StateRecord> response(
      PluginStateStore.StateRecord record) {
    return ResponseEntity.ok()
        .cacheControl(CacheControl.noStore())
        .eTag(Long.toString(record.version()))
        .body(record);
  }
}

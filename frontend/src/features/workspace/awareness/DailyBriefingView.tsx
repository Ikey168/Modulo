...
import { EMPTY_BRIEFING, EMPTY_WATCHLISTS, groupStories, preferredSignals, matches, validateBriefing, validateWatchlists } from './model';
...
export default function DailyBriefingView({ navigateView }: WorkspaceViewProps) {
  ...
  const grouped = useMemo(() => groupStories(preferredSignals(signals, store.value.delivery)), [signals, store.value.delivery]);
  ...
  return <ToolPage title="Daily Briefing" notice={<>{store.notice}{topics.notice}{newsletters.notice}</>}>{action.alert}
    ...
    <p className="text-sm text-muted-foreground">Your synchronized feed items, newsletter issues, and detected web changes. Tracking variants of article URLs and identical content are grouped together. Repeated delivery does not reset review status.</p>
    ...
    <div className="divide-y divide-border">{stories.slice(0, limit).map(story => {
      ...
      return <article key={story.key} className="space-y-3 py-4"><StoryContent story={story} navigateView={navigateView}/>{story.signals[0].url && <label className="block text-sm">Preferred delivery for {new URL(story.signals[0].url).hostname}<select className={field} disabled={!store.ready || action.busy} value={store.value.delivery?.[new URL(story.signals[0].url).hostname] || ''} onChange={event => { const source = event.target.value; const host = new URL(story.signals[0].url).hostname; void action.run(() => store.save(previous => { const delivery = { ...previous.delivery }; if (source) delivery[host] = source as 'Feeds' | 'Newsletters' | 'Web Watch'; else delete delivery[host]; return { ...previous, delivery }; })); }}><option value="">All channels</option>{['Feeds', 'Newsletters', 'Web Watch'].map(source => <option key={source}>{source}</option>)}</select></label>}{watchNames.length > 0 && <p className="text-xs text-muted-foreground">Watching: {watchNames.join(', ')}</p>}
    ...
    {Object.keys(store.value.delivery || {}).length > 0 && <details><summary className="cursor-pointer text-sm">Delivery preferences</summary><p className="text-sm">Other channels are hidden here. To stop delivery itself, unsubscribe or disable notifications at the source.</p>{Object.entries(store.value.delivery || {}).map(([host, source]) => <p key={host} className="text-sm">{host}: {source} <button className="underline" disabled={!store.ready || action.busy} onClick={() => void action.run(() => store.save(previous => { const delivery = { ...previous.delivery }; delete delivery[host]; return { ...previous, delivery }; }))}>Reset</button></p>)}</details>}

...
describe('Awareness intake', () => {
  ...
  it('uses the final watch URL for provenance when a redirect was followed', () => {
    const redirected = { id: 'redirected', title: 'Redirected watch', status: 'Changed', values: { url: 'https://example.org/old', sourceUrl: 'https://example.org/current', changeSummary: '1 added, 0 removed line; minor change.' } } as unknown as LifeRecord;
    expect(collectSignals([], [redirected], []).find(item => item.id === 'watch:redirected')?.url).toBe('https://example.org/current');
  });

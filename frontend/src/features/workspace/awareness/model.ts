...
export interface Briefing { reviewed: Record<string, string>; deadline: number; day: string; delivery?: Record<string, Signal['source']> }
...
export function collectSignals(feeds: LifeRecord[], watches: LifeRecord[], newsletters: Newsletter[]): Signal[] {
  return [
    ...
    ...watches.filter(item => item.status === 'Changed').map(item => ({ id: `watch:${item.id}`, source: 'Web Watch' as const, title: item.title, body: item.values.diff ?? item.values.changeSummary ?? 'The watched page changed.', url: safeUrl(item.values.url ?? ''), date: item.values.lastChanged ?? item.values.lastChecked ?? '', route: 'web-watch' })),
...
export function groupStories(signals: Signal[]): Story[] {
  ...
  for (const signal of signals) { const key = canonicalUrl(signal.url) || `content:${changeMarker(JSON.stringify([signal.title.trim().toLocaleLowerCase().replace(/\s+/g, ' '), signal.body.trim() || signal.id]))}`; groups.set(key, [...(groups.get(key) ?? []), signal]); }
  return [...groups].map(([key, members]) => ({ key, title: members[0].title, signals: members, revision: changeMarker(JSON.stringify([...new Set(members.map(item => JSON.stringify([item.title.trim(), item.body.trim()])) )].sort())) }))
...
/** One chosen delivery channel per publisher, without deleting the original evidence. */
export function preferredSignals(signals: Signal[], delivery: Briefing['delivery'] = {}): Signal[] {
  return signals.filter(signal => { const url = safeUrl(signal.url); const preferred = url ? delivery[new URL(url).hostname] : undefined; return !preferred || preferred === signal.source; });
}

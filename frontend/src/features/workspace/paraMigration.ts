...
export async function previewNotionFiles(files: File[]): Promise<NotionImportPreview> {
  ...
  for (const file of files) {
    ...
    for (const row of parseCsv(source)) {
      ...
      if (kind === 'goals') data.goals.push({ id, sourceId, title, level: mapGoalLevel(value(row, 'level')), status: mapGoalStatus(status), horizon: value(row, 'horizon', 'quarter') || undefined, progress: Number(value(row, 'progress').replace('%', '')) || 0, projectIds: [], areaIds: [], archivedAt: archived(status, file.name) });
...
function recoverRelations(data: ParaData, drafts: Array<{ kind: NonNullable<ReturnType<typeof kindOf>>; row: CsvRow; id: string }>) {
  ...
  for (const draft of drafts) {
    ...
    if (draft.kind === 'goals') { const item = data.goals.find((x) => x.id === draft.id); if (item) { item.projectIds = projects; item.areaIds = areas; } }
  }
}

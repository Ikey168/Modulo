...
export interface ParaTask { id: string; title: string; status: TaskStatus; priority: Priority; energy: Energy; tags?: string[]; context?: string; projectId?: string; areaId?: string; blockId?: DayBlockId; doDate?: string; deadline?: string; archivedAt?: string; sourceId?: string; externalRefs?: ExternalRef[]; }
export interface ParaGoal { id: string; title: string; level: GoalLevel; status: GoalStatus; horizon?: string; progress: number; projectIds: string[]; areaIds: string[]; archivedAt?: string; sourceId?: string; externalRefs?: ExternalRef[]; }
...
export const REVIEW_CHECKLIST = [
  ...
  'Check whether Projects still serve active Arcs',
...
export function parsePara(value: unknown): ParaData {
  ...
  return keepUnknown(raw, {
    ...
    tasks: arr(raw.tasks).filter((x) => valid(x, 'title')).map((x) => { const v = record(x); return keepUnknown(v, { id: str(v.id), title: str(v.title), status: pick(v.status, TASK_STATUSES, 'Inbox'), priority: pick(v.priority, PRIORITIES, 'P3'), energy: pick(v.energy, ENERGIES, 'Medium'), context: str(v.context) || undefined, projectId: str(v.projectId) || undefined, areaId: str(v.areaId) || undefined, blockId: optionalPick(v.blockId, DAY_BLOCKS.map((block) => block.id)), doDate: str(v.doDate) || undefined, deadline: str(v.deadline) || undefined, archivedAt: str(v.archivedAt) || undefined, sourceId: str(v.sourceId) || undefined, externalRefs: arr(v.externalRefs) as ExternalRef[] }); }),
    goals: arr(raw.goals).filter((x) => valid(x, 'title')).map((x) => { const v = record(x); return keepUnknown(v, { id: str(v.id), title: str(v.title), level: pick(v.level, GOAL_LEVELS, 'Goal'), status: pick(v.status, GOAL_STATUSES, 'Emerging'), horizon: str(v.horizon) || undefined, progress: Math.max(0, Math.min(100, typeof v.progress === 'number' ? v.progress : Number(v.progress) || 0)), projectIds: strings(v.projectIds), areaIds: strings(v.areaIds), archivedAt: str(v.archivedAt) || undefined, sourceId: str(v.sourceId) || undefined, externalRefs: arr(v.externalRefs) as ExternalRef[] }); }),
...
export function reviewSignals(data: ParaData): string[] {
  ...
  for (const goal of data.goals.filter((g) => !g.archivedAt && g.status === 'Active' && g.projectIds.every((id) => !activeProjectIds.has(id)))) signals.push(`Active arc without an active project: ${goal.title}`);
  return signals;
}

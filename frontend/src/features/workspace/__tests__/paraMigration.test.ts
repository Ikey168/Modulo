...
describe('Notion PARA migration', () => {
  ...
  it('imports arcs with several areas and ignores project relations', async () => {
    const preview = await previewNotionFiles([
      file('Areas.csv', 'Name,Status\nHealth,Growing\nLearning,Growing\n'),
      file('Projects.csv', 'Name,Status\nTraining Block,Active\n'),
      file('Arcs.csv', 'Name,Status,Areas,Projects,Quarter\nSustainable momentum,Active,"Health, Learning",Training Block,2026 Q4\n'),
    ]);
    expect(preview.data.goals[0]).toMatchObject({
      title: 'Sustainable momentum',
      horizon: '2026 Q4',
      areaIds: preview.data.areas.map((area) => area.id),
    });
    expect(preview.data.goals[0]).not.toHaveProperty('projectIds');
  });
});

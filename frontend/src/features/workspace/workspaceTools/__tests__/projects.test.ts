...
import { attachProjectNote, bundleNoteIds, detachProjectNote, newProject, projectBundle, projectTasks, remapProjectBundle, toggleProjectTask, updateProjectTaskTags, validateProjectBundle, validateProjects } from '../projects';
...
describe('project workspaces', () => {
  ...
  it('parses and updates device tags without changing the checklist state', () => {
    const taggedNote: CoreNote = { ...note, content: '- [ ] Install the bridge due:: 2026-10-01 tags:: device:pi5, matrix\n' };
    const [task] = projectTasks([taggedNote]);
    expect(task.tags).toEqual(['device:pi5', 'matrix']);
    expect(task.title).toBe('Install the bridge due:: 2026-10-01');
    expect(updateProjectTaskTags(task, taggedNote.content, ['device:pi5', 'ssh'])).toBe('- [ ] Install the bridge due:: 2026-10-01 tags:: device:pi5, ssh\n');
    expect(updateProjectTaskTags(task, taggedNote.content, [])).toBe('- [ ] Install the bridge due:: 2026-10-01\n');
  });

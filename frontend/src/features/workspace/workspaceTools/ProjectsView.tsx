...
import { DEVICE_TAG_OPTIONS, deviceTagLabel, parseTaskTags, serializeTaskTags } from '../../taskTags';
import { attachProjectNote, detachProjectNote, newProject, PROJECT_STATUSES, projectTasks, toggleProjectTask, updateProjectTaskTags, type ProjectTask, type ProjectWorkspace } from './projects';
import { useProjectData } from './useProjectData';

const sections = ['Overview', 'Notes', 'Tasks', 'Decisions', 'Runbooks', 'Evidence'] as const;

function ProjectTaskTagsEditor({
  task,
  editable,
  onSave,
}: {
  ...
  const taskTags = serializeTaskTags(task.tags);
  const [value, setValue] = useState(taskTags);

  useEffect(() => {
    setValue(taskTags);
  }, [task.line, task.original, taskTags]);

  const toggleDeviceTag = (tag: string) => {
    const tags = parseTaskTags(value);
    setValue(serializeTaskTags(tags.some(candidate => candidate.toLocaleLowerCase() === tag)
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-1.5">
      {task.tags.map(tag => (
        <span key={tag} className="rounded border border-border px-1.5 py-0.5 text-xxs text-muted-foreground">
          {tag.startsWith('device:') ? deviceTagLabel(tag) : tag}
        </span>
      ))}
      <details className="text-xs">
        <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
          {task.tags.length ? 'Edit tags' : 'Add tags'}
        </summary>
        <form className="mt-2 grid gap-2 rounded border border-border bg-background p-2" onSubmit={event => { event.preventDefault(); void onSave(parseTaskTags(value)); }}>
          <label className="text-xs text-muted-foreground">
            Tags
            <input className={`${field} mt-1 block w-full min-w-56`} value={value} onChange={event => setValue(event.target.value)} placeholder="device:pi5, zerotier" />
          </label>
          <div className="flex max-w-xl flex-wrap gap-1.5" aria-label="Device tags">
            {DEVICE_TAG_OPTIONS.map(option => {
              const selected = parseTaskTags(value).some(tag => tag.toLocaleLowerCase() === option.value);
              return <button key={option.value} type="button" aria-pressed={selected} disabled={!editable} onClick={() => toggleDeviceTag(option.value)} className={`rounded border px-1.5 py-1 text-xxs ${selected ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-border-strong'}`}>{option.label}</button>;
            })}
          </div>
          <button className={`${field} w-fit`} type="submit" disabled={!editable}>Save tags</button>
        </form>
      </details>
    </div>
  );
...
export default function ProjectsView({ data, onOpenNote }: WorkspaceViewProps) {
  ...
  const [taskTitle, setTaskTitle] = useState(''); const [taskDue, setTaskDue] = useState(''); const [taskTags, setTaskTags] = useState(''); const [taskNote, setTaskNote] = useState('');
  ...
  const select = (id: string) => { setParams({ project: id }); setDraft(undefined); setNoteId(''); setTaskNote(''); setDecisionId(''); setNoteTitle(''); setNoteBody(''); setTaskTitle(''); setTaskDue(''); setTaskTags(''); setDecisionTitle(''); setChoice(''); setExpected(''); setReviewDate(''); };
  ...
  return <ToolPage title="Project Workspaces" notice={<>{stores.projects.notice}{stores.decisions.notice}{stores.runbooks.notice}</>}>
    ...
    {project && !draft && <>
      ...
      {section === 'Tasks' && <section className="space-y-4" aria-label="Project tasks"><p className="text-sm">Tasks are checklists in project notes. Add comma-separated tags, using a <code>device:…</code> tag to show where the work happens. Procedure checklists stay in Runbooks.</p>
        <ul className="divide-y divide-border">{tasks.map(task => <li key={`${task.note.id}:${task.line}`} className="flex items-start gap-3 py-3"><input type="checkbox" aria-label={`Complete ${task.title}`} className="mt-1" checked={task.done} disabled={!editable} onChange={() => void action.run(async () => { await replaceNote(task.note, { title: task.note.title, content: toggleProjectTask(task, bodyOf(task.note)) }); await data.refresh(); })} /><div className="min-w-0"><p className="break-words">{task.title}</p><button className="text-sm underline" onClick={() => onOpenNote(task.note.id)}>{task.note.title}</button><ProjectTaskTagsEditor task={task} editable={editable} onSave={tags => action.run(async () => { await replaceNote(task.note, { title: task.note.title, content: updateProjectTaskTags(task, bodyOf(task.note), tags) }); await data.refresh(); })} /></div></li>)}</ul>
        {!tasks.length && <p>No project tasks yet.</p>}
        <form onSubmit={event => { event.preventDefault(); void action.run(async () => { const note = notes.find(note => note.id === Number(taskNote)); if (!note || project.procedureIds.includes(note.id)) throw new Error('Choose a project note for this task.'); const tags = serializeTaskTags(taskTags); const metadata = `${taskDue ? ` due:: ${taskDue}` : ''}${tags ? ` tags:: ${tags}` : ''}`; const markdown = `${bodyOf(note).trimEnd()}\n- [ ] ${taskTitle.trim().replace(/[\r\n]/g, ' ')}${metadata}\n`; await replaceNote(note, { title: note.title, content: markdown }); await data.refresh(); setTaskTitle(''); setTaskTags(''); }); }}><fieldset disabled={!editable} className="space-y-3"><label className="block">Task<input className={`${field} block w-full`} required maxLength={1000} value={taskTitle} onChange={event => setTaskTitle(event.target.value)} /></label><label className="block">Task tags<input className={`${field} block w-full`} maxLength={255} value={taskTags} onChange={event => setTaskTags(event.target.value)} placeholder="device:pi5, zerotier" /><span className="mt-1 block text-xs text-muted-foreground">Use the device tags below or enter additional tags.</span></label><div className="flex flex-wrap gap-1.5" aria-label="New task device tags">{DEVICE_TAG_OPTIONS.map(option => { const selected = parseTaskTags(taskTags).some(tag => tag.toLocaleLowerCase() === option.value); return <button key={option.value} type="button" aria-pressed={selected} onClick={() => setTaskTags(serializeTaskTags(selected ? parseTaskTags(taskTags).filter(tag => tag.toLocaleLowerCase() !== option.value) : [...parseTaskTags(taskTags), option.value]))} className={`rounded border px-1.5 py-1 text-xxs ${selected ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-border-strong'}`}>{option.label}</button>; })}</div><div className="flex flex-wrap gap-3"><label>Task note<select aria-label="Task note" className={`${field} block max-w-full`} value={taskNote} onChange={event => setTaskNote(event.target.value)}><option value="">Choose project note</option>{notes.filter(note => !project.procedureIds.includes(note.id)).map(note => <option key={note.id} value={note.id}>{note.title}</option>)}</select></label><label>Task due date<input className={`${field} block`} type="date" value={taskDue} onChange={event => setTaskDue(event.target.value)} /></label></div><button className={field} disabled={!taskTitle.trim() || !taskNote}>Add task</button></fieldset></form>
      </section>}

...
export interface ProjectTask { note: CoreNote; line: number; original: string; title: string; done: boolean; due?: string }
export function projectTasks(notes: CoreNote[]): ProjectTask[] {
  return notes.flatMap(note => {
    ...
    return bodyOf(note).split('\n').flatMap((original, line) => {
      ...
      return match ? [{ note, line, original, title: match[2], done: match[1] !== ' ', due: /(?:due::\s*|📅\s*)(\d{4}-\d{2}-\d{2})/.exec(match[2])?.[1] }] : [];
    });
  });
}

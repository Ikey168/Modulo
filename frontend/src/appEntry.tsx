...
import { startAndroidLifecycle } from './services/androidLifecycle';
import { Capacitor } from '@capacitor/core';

void startAndroidLifecycle().catch(error => console.error('Android lifecycle unavailable:', error));
...
if (import.meta.env.PROD && Capacitor.getPlatform() !== 'android' && 'serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('/sw.js')
      .then((registration) => {
        console.log('SW registered: ', registration);
        void navigator.serviceWorker.ready.then(active => {
          active.active?.postMessage({ type: 'CACHE_LOADED_ASSETS', urls: performance.getEntriesByType('resource').map(entry => entry.name) });
        });
  ...
  // Dev and packaged Android assets should use their current build, not an
  // older service worker from the WebView's persistent profile.
